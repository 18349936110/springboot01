...
import com.fjj.admin.service.EmailService;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
     ...
     * @return Result信息
     */
    public Msg checkUserAndEmail(User user) {
        //获取前端传来的数据
            ...
            if (email.equals(storeEmail)) {
                ...
                if (resetPasswordService.isExistUserId(userId)) {
                    ...
                    if (rp.getIsEffective() == 1) {
                        ...
                        return Msg.fail().add("msg","当日账号密码修改次数超过上限，请明天重试！");
                    }
                    try{
                ...
                String content = "<html>\n" +
                        "<body>\n" +
                        "<h3>hello! 忘记密码啦？!</h3>\n" +
                        "<p>" + "用户" + username + ": 你好"+"<br/>" + "你正在lkq宠物医院平台进行重置密码操作<br/>" +
                        "您本次重置密码的验证码为<br/>" +
                        "<p style=\"font-size:24px; color: #409EFF\">" + code + "</p>" +
                        "<br/>请在5分钟之内填写验证码"+
                        "<br/>如果非本人操作，请忽略本邮件, 如有疑问，欢迎致信1242061129@qq.com" +
                        "</P>" +
                        "</body>\n" +
                        "</html>\n";
                //发送Html邮件时间相对较长
                emailService.sendHtmlMail(email, title, content);
                return Msg.success().add("usernameMessage",username);
            }
            return Msg.fail().add("msg","用户邮箱号输入错误，请重新输入");
        }
        return Msg.fail().add("msg","该用户未注册，请先注册账号");
    }
     ...
     * @return Result信息
     */
    public Msg checkCode(String code, String username) {
        int userId = findIdByUsername(username);
        ResetPassword rp = resetPasswordService.getOneByUserId(userId);
        //获取当前时间
        Date currentTime = new Date();
        if (currentTime.after(rp.getDeadline())) {
            return Msg.fail().add("msg","验证时间已过,请刷新界面,从头开始重置密码！");
        }else {
            if (rp.getCode().equals(code)) {
                return Msg.success().add("验证码匹配正确", username);
            }
            return Msg.fail().add("msg","验证码匹配错误!");
        }
    }
     ...
     * @return Result信息
     */
    public Msg resetPassword(String username, String password) {
        int userId = findIdByUsername(username);
